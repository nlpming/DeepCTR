...
from deepfm_seq_config import read_file, generate_sparse_dict, get_sparse_index, generate_dense_dict, get_dense_norm
from deepfm_seq_config import generate_varlen_sparse_dict, get_varlen_sparse_index

def build_deepfm_seq_model(sparse_features, dense_features, varlen_sparse_features,
    ...
    # 加入varlen sparse features
    varlen_sparse_feature_columns = [VarLenSparseFeat(SparseFeat(feat, vocabulary_size=len(varlen_sparse_dict[feat])+2,
                                                      embedding_dim=params["embedding_dim"], embedding_name=feat),
                                                      maxlen=params["maxlen"]) for feat in varlen_sparse_features]
    dnn_feature_columns += varlen_sparse_feature_columns
...
if __name__ == "__main__":
    ...
    sparse_dict_path = os.path.join(data_path, 'data/deepfm_seq_sparse_dict.json')
    dense_dict_path = os.path.join(data_path, 'data/deepfm_seq_dense_dict.json')
    ...
    # 序列离散特征id编码
    varlen_sparse_dict = {}
    generate_varlen_sparse_dict(training_data, varlen_sparse_features, output_file=varlen_sparse_dict_path)
    ...
    for feat in varlen_sparse_features:
        training_data[feat] = training_data[feat].apply(get_varlen_sparse_index, args=(varlen_sparse_dict, feat, params['maxlen']))
